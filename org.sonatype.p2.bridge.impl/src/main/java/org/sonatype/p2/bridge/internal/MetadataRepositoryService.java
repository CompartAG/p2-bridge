...
public class MetadataRepositoryService extends AbstractService implements MetadataRepository {
    ...
    private IMetadataRepository getOrCreateRepository(final URI location, final String name,
            final Map<String, String> properties, final IMetadataRepositoryManager manager) throws ProvisionException {
        ...
        try {
            ...
            repository = manager.createRepository(location, name, IMetadataRepositoryManager.TYPE_SIMPLE_REPOSITORY,
                    properties);
        }
        return repository;
    }
    ...
    public Collection<IUIdentity> getGroupIUs(final URI... metadataRepositories) {
        ...
        try {
            ...
            final IQueryResult<IInstallableUnit> results = QueryUtil.compoundQueryable(repositories).query(
    ...
    public Collection<IUIdentity> getVersions(final Collection<String> ius, final boolean onlyUpdates,
        ...
        try {
            ...
            for (final String spec : ius) {
                ...
                final IQueryResult<IInstallableUnit> results = QueryUtil.compoundQueryable(repositories).query(query,
                        null);
    ...
    public void createProxyRepository(final URI location, final String username, final String password,
        ...
        try {
            ...
            try {
                ...
                final IMetadataRepository localRepository = getOrCreateRepository(destination,
                        remoteRepository.getName(), properties, manager);
    ...
    private Collection<IMetadataRepository> getRepositories(final IMetadataRepositoryManager manager,
            final URI... locations) throws ProvisionException {
    ...
    private IMetadataRepositoryManager getManager(final URI location) throws ProvisionException {
        if (getProvider() == null) {
    ...
    private void addArtifacts(final InstallableUnit unit, final InstallableUnitDescription description) {
        ...
        for (final InstallableUnitArtifact unitA : unitAs) {
            final ArtifactKey artifactKey = new ArtifactKey(unitA.getClassifier(), unitA.getId(),
                    Version.parseVersion(unitA.getVersion()));
    ...
    private IRequirement createRequirement(final RequiredCapability unitRC) {
        ...
        if (unitRC.getMatch() == null) {
            requirement = MetadataFactory
                .createRequirement(unitRC.getNamespace(), unitRC.getName(), new VersionRange(unitRC.getRange()),
                        unitRC.getFilter(), unitRC.isOptional(), unitRC.isMultiple(), unitRC.isGreedy());
        } else {
            ...
            if (unitRC.getMatchParameters() == null) {
                ...
                final IExpression[] arrayExpr = ExpressionUtil.getOperands(ExpressionUtil.parse(unitRC
                    .getMatchParameters()));
            ...
            IMatchExpression<IInstallableUnit> filter = org.eclipse.equinox.internal.p2.metadata.InstallableUnit
                .parseFilter(unitRC.getFilter());
            requirement = MetadataFactory.createRequirement(matchExpr, filter, unitRC.getMin(), unitRC.getMax(),
    ...
    private void addProvidedCapabilities(final InstallableUnit unit, final InstallableUnitDescription description) {
        ...
        for (final ProvidedCapability unitPC : unitPCs) {
            if (unitPC != null) {
                final IProvidedCapability providedCapability = MetadataFactory.createProvidedCapability(
                        unitPC.getNamespace(), unitPC.getName(), Version.create(unitPC.getVersion()));
    ...
    private void addTouchpointType(final InstallableUnit unit, final InstallableUnitDescription description) {
        ...
        final ITouchpointType touchpointType = MetadataFactory.createTouchpointType(unitTP.getId(),
                Version.create(unitTP.getVersion()));
    ...
    private void addUpdateDescriptor(final InstallableUnit unit, final InstallableUnitDescription description) {
        ...
        final int severity = unitUD.getSeverity() == null ? IUpdateDescriptor.NORMAL : Integer.valueOf(unitUD
            .getSeverity());

        final IUpdateDescriptor updateDescriptor = MetadataFactory.createUpdateDescriptor(unitUD.getId(),
                new VersionRange(unitUD.getRange()), severity, null);
